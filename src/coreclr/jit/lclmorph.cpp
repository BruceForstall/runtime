                ...
                bool AddOffset(Value& val, unsigned addOffset)
                {
                    ...
                    if (val.IsAddress())
                    {
                        ClrSafeInt<unsigned> newOffset =
                            ClrSafeInt<unsigned>(val.m_offset) + ClrSafeInt<unsigned>(addOffset);
                ...
                case GT_RETURN:
                    if (TopValue(0).Node() != node)
                    {
                        ...
                        if (retVal->OperIs(GT_LCL_VAR))
                        {
                            ...
                            // I have a change that introduces more precise and effective solution for that, but it
                            // would be merged separately.
        ...
        void EscapeAddress(Value& val, GenTree* user)
        {
            ...
            if (!hasHiddenStructArg)
            {
                m_compiler->lvaSetVarAddrExposed(varDsc->lvIsStructField
                                                     ? varDsc->lvParentLcl
                                                     : lclNum DEBUGARG(AddressExposedReason::ESCAPE_ADDRESS));
            }
        ...
        void MorphLocalIndir(GenTree** use, unsigned lclNum, unsigned offset, GenTree* user)
        {
            ...
            switch (transform)
            {
                        ...
                        switch (elementType)
                        {
                            case TYP_FLOAT:
                                {
                                    ...
                                    hwiNode =
                                        m_compiler->gtNewSimdGetElementNode(elementType, lclNode, indexNode,
                            ...
                            case TYP_SIMD12:
                                {
                                    ...
                                    hwiNode = m_compiler->gtNewSimdHWIntrinsicNode(elementType, lclNode,
                            ...
                            case TYP_SIMD32:
#endif
                                {
                                    ...
                                    {
                                        hwiNode =
                                            m_compiler->gtNewSimdGetLowerNode(elementType, lclNode, CORINFO_TYPE_FLOAT,
                                                                              genTypeSize(varDsc));
                                    }
                                    else
                                    {
                                        ...
                                        hwiNode =
                                            m_compiler->gtNewSimdGetUpperNode(elementType, lclNode, CORINFO_TYPE_FLOAT,
                                                                              genTypeSize(varDsc));
                                    }
                        ...
                        switch (elementType)
                        {
                            case TYP_FLOAT:
                                {
                                    GenTree* indexNode = m_compiler->gtNewIconNode(offset / genTypeSize(elementType));
                                    hwiNode =
                                        m_compiler->gtNewSimdWithElementNode(varDsc->TypeGet(), simdLclNode, indexNode,
                            ...
                            case TYP_SIMD12:
                                {
                                    ...
                                    // We inverse the operands here and take elementNode as the main value and
                                    // simdLclNode[3] as the new value. This gives us a new TYP_SIMD16 with all elements
                                    // in the right spots
                                    GenTree* indexNode = m_compiler->gtNewIconNode(3, TYP_INT);
                                    hwiNode = m_compiler->gtNewSimdWithElementNode(TYP_SIMD16, elementNode, indexNode,
                                                                                   simdLclNode, CORINFO_TYPE_FLOAT, 16);
                                    break;
                                }
                            case TYP_SIMD8:
#if defined(FEATURE_SIMD) && defined(TARGET_XARCH)
                            ...
                            case TYP_SIMD32:
#endif
                                {
                                    ...
                                    if (offset == 0)
                                    {
                                        hwiNode = m_compiler->gtNewSimdWithLowerNode(varDsc->TypeGet(), simdLclNode,
                                                                                     elementNode, CORINFO_TYPE_FLOAT,
                                                                                     genTypeSize(varDsc));
                                    }
                                    else
                                    {
                                        assert(offset == genTypeSize(elementType));
                                        hwiNode = m_compiler->gtNewSimdWithUpperNode(varDsc->TypeGet(), simdLclNode,
                                                                                     elementNode, CORINFO_TYPE_FLOAT,
                                                                                     genTypeSize(varDsc));
                                    }
                ...
                case IndirTransform::LclFld:
                    ...
                    {
                        // The general invariant in the compiler is that whoever creates a LCL_FLD node after local
                        // morph must mark the associated local DNER. We break this invariant here, for STRUCT fields,
                        // to allow global morph to transform these into enregisterable LCL_VARs, applying DNER
                        // otherwise.
