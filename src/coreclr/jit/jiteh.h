...
struct EHblkDsc
{
        ...
        // How nested is the try/handler within other *handlers* - 0 for outermost clauses, 1 for nesting with a
        // handler, etc.
        ...
        // in the try.  This is normally "ebdHndBeg", unless the try region has a filter, in which case that is
        // returned. (This is, in some sense, the "true handler," at least in the sense of control flow.  Note that we
        // model the transition from a filter to its handler as normal, non-exceptional control flow.)
