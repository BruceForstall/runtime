...
//
template <class T> RegisterType regType(T type)
{
...
class LinearScan : public LinearScanInterface
{
    ...
    public:
        ...
        template <bool localVarsEnregistered> void buildIntervals();
...
// This is where the actual assignment is done
        ...
        template <bool localVarsEnregistered> void resolveRegisters();
...
#error Unsupported or unset target architecture
    ...
    private:
        ...
        template <bool localVarsEnregistered> void identifyCandidates();
        ...
        template <bool localVarsEnregistered> void processBlockEndAllocation(BasicBlock* current);
...
// Otherwise, the Build methods should obtain the killMask from the appropriate method above.
    ...
    private:
        ...
        template <bool needsConsecutiveRegisters = false> regNumber assignCopyReg(RefPosition* refPosition);
...
#include "lsra_score.h"
        ...
        void lsraGetOperandString(GenTree*          tree,
...
#define allAvailableRegs (availableIntRegs | availableFloatRegs)
        ...
        // Set of variables exposed on finally edges. These must be zero-init if they are refs or if compInitMem is
        // true.
...
class RefPosition
{
    public:
        ...
        // ref positions that lower/codegen has indicated as reg optional and is considered a contained memory operand
        // if no reg is allocated.
        ...
        //    record the new register location as the new "home" location of the lclVar. However, if there is a
        //    conflicting use at the same location (e.g. lclVar V1 is in rdx and needs to be in rcx, but V2 needs to be
        //    in rdx), then we need an explicit move.
        ...
        unsigned char writeThru : 1; // true if this var is defined in a register and also spilled. spillAfter must NOT
                                     // be set.
