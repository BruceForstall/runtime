...
//                subclasses.
template <typename TKey> struct HashTableInfo
{
...
//                       typed keys.
template <typename TKey> struct HashTableInfo<TKey*>
{
        static bool Equals(const TKey* x, const TKey* y)
        {
            return x == y;
...
//                     typed keys.
template <> struct HashTableInfo<int>
{
...
//                          typed keys.
template <> struct HashTableInfo<unsigned>
{
...
//                          typed keys.
template <> struct HashTableInfo<ssize_t>
{
    ...
    private:
        ...
        void Resize()
        {
            ...
            for (unsigned currentIndex = 0; currentIndex < m_numBuckets; currentIndex++)
            {
                ...
                bool inserted = Insert(newBuckets, newNumBuckets, currentBucket->m_hash, currentBucket->m_key,
                                       currentBucket->m_value);
