        ...
        class FatPointerCallTransformer final : public Transformer
        {
            ...
            protected:
                ...
                virtual void CreateCheck(uint8_t checkIdx)
                {
                    ...
                    GenTree* fatPointerAnd =
                        compiler->gtNewOperNode(GT_AND, TYP_I_IMPL, fptrAddressCopy, fatPointerMask);
        ...
        class GuardedDevirtualizationTransformer final : public Transformer
        {
            ...
            protected:
                ...
                virtual void CreateCheck(uint8_t checkIdx)
                {
                    ...
                    else
                    {
                        ...
                        JITDUMP("Level %u Check block " FMT_BB " success likelihood " FMT_WT "\n", checkIdx,
                                checkBlock->bbNum, checkLikelihood);
                ...
                virtual void FixupRetExpr()
                {
                    ...
                    if (!origCall->TypeIs(TYP_VOID))
                    {
                        ...
                        JITDUMP("Linking GT_RET_EXPR [%06u] to refer to temp V%02u\n",
                                compiler->dspTreeID(inlineInfo->retExpr), returnTemp);
                ...
                void DevirtualizeCall(BasicBlock* block, uint8_t candidateId)
                {
                    ...
                    {
                        GenTree* offset =
                            compiler->gtNewIconNode((ssize_t)compiler->eeGetEEInfo()->offsetOfDelegateInstance,
                                                    TYP_I_IMPL);
                    ...
                    if (clsHnd != NO_CLASS_HANDLE)
                    {
                        ...
                        const bool explicitTailCall =
                            (call->AsCall()->gtCallMoreFlags & GTF_CALL_M_EXPLICIT_TAILCALL) != 0;
                    ...
                    else
                    {
                        JITDUMP("[%06u] is%s in tail position and is%s recursive\n", compiler->dspTreeID(call),
                                call->CanTailCall() ? "" : " not",
                                compiler->gtIsRecursiveCall(methodHnd) ? "" : " not");
                    }

                    // If the devirtualizer was unable to transform the call to invoke the unboxed entry, the inline
                    // info we set up may be invalid. We won't be able to inline anyways. So demote the call as an
                    // inline candidate.
                    //
                    ...
                    if ((unboxedMethodHnd != nullptr) && (methodHnd != unboxedMethodHnd))
                    {
                        ...
                        //
                        JITDUMP(
                            "Devirtualization was unable to use the unboxed entry; so marking call (to boxed entry) as not "
                            "inlineable\n");
                ...
                virtual void CreateElse()
                {
                    ...
                    else
                    {
                        ...
                        // We aren't converting checkBlock to a BBJ_COND. Its successor edge likelihood should
                        // remain 1.0.
                        //
                        ...
                        if (root->IsCall())
                        {
                                ...
                                (call->GetGDVCandidateInfo(0)->likelihood >= gdvChainLikelihood))
                            {
                                JITDUMP(
                                    ...
                                    compiler->dspTreeID(call), call->GetGDVCandidateInfo(0)->likelihood,
