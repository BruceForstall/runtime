    ...
    private:
        ...
        void FinalizeInit(DecompositionStatementList* statements)
        {
            ...
            for (int i = 0; i < m_entries.Height(); i++)
            {
                ...
                assert((entry.ToReplacement >= firstRep) &&
                       (entry.ToReplacement < firstRep + agg->Replacements.size()));
            ...
            else if (remainderStrategy.Type == RemainderStrategy::Primitive)
            {
                ...
                GenTree* store = storeAccess.CreateStore(remainderStrategy.PrimitiveOffset,
                                                         remainderStrategy.PrimitiveType, value, m_compiler);
                statements->AddStatement(store);
            }
        }
        ...
        //   deaths            - Liveness information for the destination; only valid if m_dstInvolvedReplacements is
        //   true. remainderStrategy - The strategy we are using for the remainder dump              - Whether to
        //   JITDUMP decisions made
        //
                          ...
                          const RemainderStrategy& remainderStrategy DEBUGARG(bool dump = false))
        {
            ...
            if (entry.FromReplacement != nullptr)
            {
                ...
                if ((remainderStrategy.Type == RemainderStrategy::FullBlock) &&
                    !entry.FromReplacement->NeedsWriteBack && (entry.ToReplacement == nullptr))
                {
#ifdef DEBUG
                    if (dump)
                    {
        ...
        class LocationAccess
        {
            ...
            public:
                ...
                GenTree* CreateRead(unsigned offs, var_types type, Compiler* comp)
                {
                    ...
                    GenTreeLclFld* fld =
                        comp->gtNewLclFldNode(m_local->GetLclNum(), type, m_local->GetLclOffs() + offs);
                ...
                GenTree* CreateStore(unsigned offs, var_types type, GenTree* src, Compiler* comp)
                {
                    ...
                    {
                        GenTreeIndir* indir =
                            comp->gtNewStoreIndNode(type, GrabAddress(offs, comp), src, GetIndirFlags(type));
                        return indir;
                    }
                           ...
                           DecompositionStatementList* statements)
        {
            ...
            else if (remainderStrategy.Type == RemainderStrategy::Primitive)
            {
                ...
                // direct access.
                unsigned srcPromField =
                    srcAccess.FindRegularlyPromotedField(remainderStrategy.PrimitiveOffset, m_compiler);
                ...
                GenTree* store =
                    storeAccess.CreateStore(remainderStrategy.PrimitiveOffset, primitiveType, src, m_compiler);
                statements->AddStatement(store);
            }
        }
};
