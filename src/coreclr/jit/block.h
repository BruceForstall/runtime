...
template <typename... MemoryKinds> inline MemoryKindSet memoryKindSet(MemoryKind memoryKind, MemoryKinds... memoryKinds)
{
...
class PredEdgeList
{
        ...
        class iterator
        {
                ...
                // operation. This is a bit conservative, but attempts to protect against callers assuming too much
                // about this iterator implementation.
                FlowEdge* m_next;
#endif
...
//
template <bool allowEdits> class PredBlockList
{
        FlowEdge* m_begin;
        ...
        class iterator
        {
                ...
                // When allowEdits=false, try to guard against the user of the iterator from modifying the predecessor
                // list being traversed: cache the edge we think should be next, then check it when we actually do the
                // `++` operation. This is a bit conservative, but attempts to protect against callers assuming too much
                // about this iterator implementation. When allowEdits=true, m_next is always used to update m_pred, so
                // changes to m_pred don't break the iterator.
                FlowEdge* m_next;
...
struct BasicBlock : private LIR::Range
{
    ...
    public:
        ...
        unsigned bbRefs; // number of blocks that can reach here, either by fall-through or a branch. If this falls to
                         // zero, the block is unreachable.
...
#define BB_MAX_WEIGHT            FLT_MAX // maximum finite weight  -- needs rethinking.
        ...
        template <typename... T> bool KindIs(BBKinds kind, T... rest) const
        {
        ...
        // SwitchTargets: convenience method for enabling range-based `for` iteration over a switch block's targets,
        // e.g.:
...
#define MAX_XCPTN_INDEX (USHRT_MAX - 1)
        ...
        // index, into the compHndBBtab table, of innermost 'try' clause containing the BB (used for raising
        // exceptions). Stored as index + 1; 0 means "no try index".
        ...
        // have a numeric index relationship that doesn't indicate nesting, whereas a more deeply nested region must
        // have a lower index than the region it is nested within. Note that if you compare a single block's bbTryIndex
        // and bbHndIndex, there is guaranteed to be a nesting relationship, since that block can't be simultaneously in
        // two sibling EH regions. In that case, "maybe" is actually "definitely".
        static bool ehIndexMaybeMoreNested(unsigned index1, unsigned index2)
        {
            ...
            {
                // index1 represents an EH region, whereas index2 is the main method. Thus, index1 is more deeply
                // nested.
                assert(index1 > 0);
                return true;
            }
...
#define handlerGetsXcptnObj(hndTyp) ((hndTyp) != BBCT_NONE && (hndTyp) != BBCT_FAULT && (hndTyp) != BBCT_FINALLY)
        ...
        union
        {
                ...
                ASSERT_TP bbAssertionOutIfTrue; // assertions available on exit along true/jump edge (BBJ_COND, local
                                                // prop)
        };
        ...
        template <typename TPosition> class Successors
        {
                Compiler*   m_comp;
                BasicBlock* m_block;
        ...
        template <typename TFunc> BasicBlockVisit VisitEHEnclosedHandlerSecondPassSuccs(Compiler* comp, TFunc func);

        template <typename TFunc> BasicBlockVisit VisitAllSuccs(Compiler* comp, TFunc func);

        template <typename TFunc> BasicBlockVisit VisitEHSuccs(Compiler* comp, TFunc func);

        template <typename TFunc> BasicBlockVisit VisitRegularSuccs(Compiler* comp, TFunc func);
...
template <> struct JitPtrKeyFuncs<BasicBlock> : public JitKeyFuncsDefEquals<const BasicBlock*>
{
    public:
...
template <bool allowEdits> inline BasicBlock* PredBlockList<allowEdits>::iterator::operator*() const
{
