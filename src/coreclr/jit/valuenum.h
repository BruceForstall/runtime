...
class ValueNumStore
{
    ...
    private:
        ...
        // return vnf(v0)
        template <typename T> static T EvalOp(VNFunc vnf, T v0);

        // returns vnf(v0, v1).
        template <typename T> T EvalOp(VNFunc vnf, T v0, T v1);

        // return vnf(v0) or vnf(v0, v1), respectively (must, of course be unary/binary ops, respectively.)
        template <typename T> static T EvalOpSpecialized(VNFunc vnf, T v0);
        template <typename T> T        EvalOpSpecialized(VNFunc vnf, T v0, T v1);

        template <typename T> static int EvalComparison(VNFunc vnf, T v0, T v1);
        ...
        template <typename T> static bool IsOverflowIntDiv(T v0, T v1);
        ...
        template <typename T> static bool IsIntZero(T v);
...
#define DEFAULT_MAP_SELECT_BUDGET 100 // used by JitVNMapSelBudget
        ...
        template <typename T, typename NumMap> inline ValueNum VnForConst(T cnsVal, NumMap* numMap, var_types varType);
    ...
    public:
        ...
        // A specialized version of VNForFunc that is used for VNF_MapStore and provides some logging when verbose is
        // set
        ...
        ValueNumPair VNPairForFunc(
            var_types typ, VNFunc func, ValueNumPair op1VN, ValueNumPair op2VN, ValueNumPair op3VN)
        {
...
#define FEATURE_VN_TRACE_APPLY_SELECTORS 1
        ...
        template <int N> struct VarTypConv
        {
        };
    ...
    private:
        ...
        template <typename T> static T CoerceTypRefToT(Chunk* c, unsigned offset);

        // Get the actual value and coerce the actual type c->m_typ to the wanted type T.
        template <typename T> FORCEINLINE T SafeGetConstantValue(Chunk* c, unsigned offset);

        template <typename T> T ConstantValueInternal(ValueNum vn DEBUGARG(bool coerce))
        {
    ...
    public:
        ...
        template <typename T> T ConstantValue(ValueNum vn)
        {
        ...
        // type "T".
        template <typename T> T CoercedConstantValue(ValueNum vn)
        {
...
#define FEATURE_VN_DUMP_FUNC_ARGS 0
    ...
    private:
        ...
        template <size_t NumArgs> struct VNDefFuncApp
        {
                VNFunc   m_func;
        ...
        struct Chunk
        {
                // If "m_defs" is non-null, it is an array of size ChunkSize, whose element type is determined by the
                // other members. The "m_numUsed" field indicates the number of elements of "m_defs" that are already
                // consumed (the next one to allocate).
                void*    m_defs;
                unsigned m_numUsed;
                ...
                // Initialize a chunk, starting at "*baseVN", for the given "typ", and "attribs", using "alloc" for
                // allocations. (Increments "*baseVN" by ChunkSize.)
                ...
                VNDefFuncAppFlexible* PointerToFuncApp(unsigned offsetWithinChunk, unsigned numArgs)
                {
                    ...
                    return reinterpret_cast<VNDefFuncAppFlexible*>(
                        (char*)m_defs +
                        offsetWithinChunk * (sizeof(VNDefFuncAppFlexible) + sizeof(ValueNum) * numArgs));
                }
        ...
        // This is the set of value numbers that have been flagged as arguments to bounds checks, in the length
        // position.
        ...
        template <size_t NumArgs> struct VNDefFuncAppKeyFuncs : public JitKeyFuncsDefEquals<VNDefFuncApp<NumArgs>>
        {
...
template <> struct ValueNumStore::VarTypConv<TYP_INT>
{
        typedef INT32 Type;
        typedef int   Lang;
};
template <> struct ValueNumStore::VarTypConv<TYP_FLOAT>
{
        typedef INT32 Type;
        typedef float Lang;
};
template <> struct ValueNumStore::VarTypConv<TYP_LONG>
{
        typedef INT64 Type;
        typedef INT64 Lang;
};
template <> struct ValueNumStore::VarTypConv<TYP_DOUBLE>
{
        typedef INT64  Type;
        typedef double Lang;
};
...
#if defined(FEATURE_SIMD)
template <> struct ValueNumStore::VarTypConv<TYP_SIMD8>
{
        typedef simd8_t Type;
        typedef simd8_t Lang;
};
template <> struct ValueNumStore::VarTypConv<TYP_SIMD12>
{
        typedef simd12_t Type;
        typedef simd12_t Lang;
};
template <> struct ValueNumStore::VarTypConv<TYP_SIMD16>
{
        typedef simd16_t Type;
        typedef simd16_t Lang;
};
#if defined(TARGET_XARCH)
template <> struct ValueNumStore::VarTypConv<TYP_SIMD32>
{
        typedef simd32_t Type;
        typedef simd32_t Lang;
};

template <> struct ValueNumStore::VarTypConv<TYP_SIMD64>
{
        typedef simd64_t Type;
        typedef simd64_t Lang;
};

template <> struct ValueNumStore::VarTypConv<TYP_MASK>
{
...
template <> struct ValueNumStore::VarTypConv<TYP_BYREF>
{
        typedef size_t Type;
        typedef void*  Lang;
};
template <> struct ValueNumStore::VarTypConv<TYP_REF>
{
...
// Get the actual value and coerce the actual type c->m_typ to the wanted type T.
template <typename T> FORCEINLINE T ValueNumStore::SafeGetConstantValue(Chunk* c, unsigned offset)
{
    switch (c->m_typ)
    {
        case TYP_REF:
...
#if defined(FEATURE_SIMD)
template <> FORCEINLINE simd8_t ValueNumStore::SafeGetConstantValue<simd8_t>(Chunk* c, unsigned offset)
{
    assert(c->m_typ == TYP_SIMD8);
    return reinterpret_cast<VarTypConv<TYP_SIMD8>::Lang*>(c->m_defs)[offset];
}

template <> FORCEINLINE simd12_t ValueNumStore::SafeGetConstantValue<simd12_t>(Chunk* c, unsigned offset)
{
    assert(c->m_typ == TYP_SIMD12);
    return reinterpret_cast<VarTypConv<TYP_SIMD12>::Lang*>(c->m_defs)[offset];
}

template <> FORCEINLINE simd16_t ValueNumStore::SafeGetConstantValue<simd16_t>(Chunk* c, unsigned offset)
{
...
#if defined(TARGET_XARCH)
template <> FORCEINLINE simd32_t ValueNumStore::SafeGetConstantValue<simd32_t>(Chunk* c, unsigned offset)
{
    assert(c->m_typ == TYP_SIMD32);
    return reinterpret_cast<VarTypConv<TYP_SIMD32>::Lang*>(c->m_defs)[offset];
}

template <> FORCEINLINE simd64_t ValueNumStore::SafeGetConstantValue<simd64_t>(Chunk* c, unsigned offset)
{
    assert(c->m_typ == TYP_SIMD64);
    return reinterpret_cast<VarTypConv<TYP_SIMD64>::Lang*>(c->m_defs)[offset];
}

template <> FORCEINLINE simdmask_t ValueNumStore::SafeGetConstantValue<simdmask_t>(Chunk* c, unsigned offset)
{
...
template <> FORCEINLINE simd8_t ValueNumStore::ConstantValueInternal<simd8_t>(ValueNum vn DEBUGARG(bool coerce))
{
    Chunk* c = m_chunks.GetNoExpand(GetChunkNum(vn));
...
template <> FORCEINLINE simd12_t ValueNumStore::ConstantValueInternal<simd12_t>(ValueNum vn DEBUGARG(bool coerce))
{
    Chunk* c = m_chunks.GetNoExpand(GetChunkNum(vn));
    assert(c->m_attribs == CEA_Const);
...
template <> FORCEINLINE simd16_t ValueNumStore::ConstantValueInternal<simd16_t>(ValueNum vn DEBUGARG(bool coerce))
{
    Chunk* c = m_chunks.GetNoExpand(GetChunkNum(vn));
    assert(c->m_attribs == CEA_Const);
...
#if defined(TARGET_XARCH)
template <> FORCEINLINE simd32_t ValueNumStore::ConstantValueInternal<simd32_t>(ValueNum vn DEBUGARG(bool coerce))
{
    Chunk* c = m_chunks.GetNoExpand(GetChunkNum(vn));
    assert(c->m_attribs == CEA_Const);
...
template <> FORCEINLINE simd64_t ValueNumStore::ConstantValueInternal<simd64_t>(ValueNum vn DEBUGARG(bool coerce))
{
    Chunk* c = m_chunks.GetNoExpand(GetChunkNum(vn));
    assert(c->m_attribs == CEA_Const);
...
template <> FORCEINLINE simdmask_t ValueNumStore::ConstantValueInternal<simdmask_t>(ValueNum vn DEBUGARG(bool coerce))
{
    Chunk* c = m_chunks.GetNoExpand(GetChunkNum(vn));
    assert(c->m_attribs == CEA_Const);
...
template <> inline size_t ValueNumStore::CoerceTypRefToT(Chunk* c, unsigned offset)
{
    return reinterpret_cast<size_t>(reinterpret_cast<VarTypConv<TYP_REF>::Type*>(c->m_defs)[offset]);
}

template <typename T> inline T ValueNumStore::CoerceTypRefToT(Chunk* c, unsigned offset)
{
